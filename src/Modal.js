...
class Modal extends React.Component {
  ...
  getChildContext() {
    return (
      this._context || (this._context = { onModalHide: this.props.onHide })
    );
  }
  ...
  componentDidMount() {
    getZIndex =
      getZIndex ||
      (() => {
        let btsModal = document.createElement('div'),
          backdrop = document.createElement('div'),
          zIndexFactor;

        btsModal.className = 'btsModal hide';
        backdrop.className = 'btsModal-backdrop hide';

        document.body.appendChild(btsModal);
        document.body.appendChild(backdrop);

        baseIndex.btsModal = +css(btsModal, 'z-index');
        baseIndex.backdrop = +css(backdrop, 'z-index');
        zIndexFactor = baseIndex.btsModal - baseIndex.backdrop;

        document.body.removeChild(btsModal);
        document.body.removeChild(backdrop);

        return (type) =>
          baseIndex[type] +
          zIndexFactor * (this.props.manager.modals.length - 1);
      })();
  }
  ...
  render() {
    ...
    let btsModal = (
      <div
        ...
        onClick={
          this.props.backdrop ? (e) => this.handleBackdropClick(e) : null
        }>
        <div
          key="btsModal"
    ...
    return (
      <BaseModal
        keyboard={keyboard}
        ref={(ref) => {
          this.btsModal = ref && ref.btsModal;
